...
当前 v0.1 只实现了简化版: researcher 根据单个 task 文件生成 `spec.md` 和 `interfaces.md`。真正的多轮 discovery 仍是 v0.1 hardening 目标。
...
| 角色 | 职责 | 当前 v0.1 状态 |
|---|---|---|
| researcher | 开发前澄清/调研,产出 `spec.md`、`interfaces.md`,后续负责 `discovery.md`、`api-probes/` | 已实现单轮 artifact 生成;未实现多轮用户澄清 |
| manager | 读取文件协议,决定 `develop` / `test` / `done` / `ask_user` | 已实现 JSON schema 决策 |
| developer | 基于冻结接口实现 `workspace/` | 已实现 |
| tester | 基于契约和验收标准测试实现 | 已实现 |
| observer | 审视 session trace,提炼 lessons/snippets | 未实现,v0.4 |
...
api-probes/        # 外部 API/SDK probe 和响应样例
snippets/          # agent 友好的复用组件池
lessons.md         # observer 提炼的经验
```
...
- role 失败尚未统一写入 `blockers.md` / error session log。
...
- 增加真正的 discovery 阶段:
  ...
  - 开发前明确 freeze
- 增加 `--turn-timeout-ms`
- 用 `AbortController` 限制单个 `thread.run()`
- role 失败时写入:
  - `blockers.md`
  - `progress.md`
  - `session-log/<timestamp>-<role>-error.json`
- CLI 在端到端失败时返回非零退出码
- 增加 blocker 路径验收 task
...
v0.1 已经证明核心技术方向可行:Codex SDK 可以作为编排底座,文件协议可以支撑多 role 串行闭环。
